...
function questionRow(q, i, sections = [], clientId = '', showRespond = false, showView = false) {
  ...
  return `<tr class="hover">
    <td class="text-center text-sm text-base-content w-8">${i+1}</td>
    <td class="text-sm max-w-sm">${esc(q.question_text||q.question||q.title||'Question')}</td>
    <td>${sLabel}</td>
    <td class="text-sm text-base-content">${q.deadline?new Date(typeof q.deadline==='number'?q.deadline*1000:q.deadline).toLocaleDateString('en-ZA',{day:'2-digit',month:'short',year:'numeric'}):'-'}</td>
...
export function renderRfiDetail(user, rfi = {}, questions = [], sections = [], engagement = null) {
  ...
  const infoItems = [
    ['Engagement', engagement?.name ? esc(engagement.name) : '<span class="text-base-content">—</span>'],
  ...
  const content = `
      ...
      <div class="card-clean-body">
        ...
        <div class="table-wrap">
          <table class="data-table" id="q-table" aria-label="Questions table">
            ...
            <tbody id="q-tbody">${qRows || `<tr><td colspan="6" role="status" aria-live="polite" style="padding:${SPACING.md}"><span class="sr-only">Loading questions…</span><div class="skeleton" style="height:40px"></div></td></tr>`}</tbody>
          </table>
        </div>
      </div>
    </div>
    ...
    ${canViewResponses ? `<div id="view-resp-dialog" class="modal" style="display:none" ><div class="modal-overlay" data-dialog-close="view-resp-dialog"></div><div class="modal-content rounded-box max-w-lg p-6" aria-labelledby="view-resp-dialog-title"><h2 class="text-lg font-semibold mb-1" id="view-resp-dialog-title">Responses</h2><p id="view-resp-qtext" class="text-sm text-base-content mb-3"></p><div id="view-resp-list" style="max-height:240px;overflow-y:auto"></div><div class="modal-action"><button data-dialog-close="view-resp-dialog" class="btn btn-ghost">Close</button></div></div></div>` : ''}
    ${isClient ? `<div id="respond-q-dialog" class="modal" style="display:none" ><div class="modal-overlay" data-dialog-close="respond-q-dialog"></div><div class="modal-content rounded-box max-w-lg p-6" aria-labelledby="respond-q-dialog-title"><h2 class="text-lg font-semibold mb-1" id="respond-q-dialog-title">Respond to Question</h2><p id="respond-q-text" class="text-sm text-base-content mb-4"></p><div class="form-group mb-3"><label class="label"><span class="label-text font-medium">Your Response</span></label><textarea id="respond-q-answer" rows="5" class="textarea textarea-solid max-w-full" placeholder="Enter your response..."></textarea></div><div class="form-group mb-4"><label class="label"><span class="label-text font-medium">Attach File (optional)</span></label><input type="file" id="respond-q-file" class="input input-solid max-w-full"/></div><input type="hidden" id="respond-q-id"/><div class="modal-action"><button data-action="submitResponse" class="btn btn-primary">Submit</button><button data-dialog-close="respond-q-dialog" class="btn btn-ghost">Cancel</button></div></div></div>` : ''}
  `;

  const script = `${TOAST}
(function(){if(!document.getElementById('rfi-detail-styles')){var style=document.createElement('style');style.id='rfi-detail-styles';style.textContent='.info-label-key{font-size:0.7rem;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:var(--color-text-muted);margin-bottom:${SPACING.xs}} .info-label-value{font-size:0.875rem;color:var(--color-text)} .tab-btn:focus-visible{outline:2px solid var(--color-primary);outline-offset:-2px}';document.head.appendChild(style)}})();
var activeSection='';
...
(function(){
  var tabBtns=document.querySelectorAll('[id^="stab-"]');
  tabBtns.forEach(function(btn,idx){
    btn.setAttribute('role','tab');
    btn.setAttribute('tabindex',btn.classList.contains('active')?'0':'-1');
    btn.setAttribute('aria-selected',btn.classList.contains('active')?'true':'false');
    btn.addEventListener('click',function(){tabBtns.forEach(b=>b.setAttribute('tabindex','-1'));btn.setAttribute('tabindex','0');btn.focus()});
  });
  var searchInput=document.querySelector('#q-table input');
