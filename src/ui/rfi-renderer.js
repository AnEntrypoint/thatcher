...
export function rfiSectionCrudPanel(engagementId, sections = []) {
  const rows = sections.map((s, i) => `<div class="rfi-section-row" draggable="true" role="button" tabindex="0" aria-label="${esc(s.name || 'Untitled')} section, ${s.question_count || 0} questions. Press Ctrl+Up or Ctrl+Down to reorder." data-id="${esc(String(s.id))}" data-idx="${i}" ondragstart="rfiDragStart(event)" ondragover="rfiDragOver(event)" ondrop="rfiDrop(event)" onkeydown="rfiSectionKey(event)"><span class="rfi-section-handle" aria-hidden="true">${RFI_HANDLE_SVG}</span><span class="rfi-section-name">${esc(s.name || 'Untitled')}</span><span class="badge-status">${s.question_count || 0} Q</span><div class="flex gap-1"><button class="btn btn-xs btn-ghost" data-action="rfiEditSection" data-args='${esc(JSON.stringify([String(s.id), String(s.name || '')]))}'>Edit</button><button class="btn btn-xs btn-ghost btn-error" data-action="rfiDeleteSection" data-args='${esc(JSON.stringify([String(s.id)]))}'>Del</button></div></div>`).join('');
  return `<style>.rfi-section-row:focus-visible{outline:2px solid var(--ds-accent,#0072e6);outline-offset:2px}</style><div class="card-clean" style="margin-bottom:1rem"><div class="card-clean-body"><div class="flex justify-between items-center mb-3"><h3 style="font-size:0.875rem;font-weight:600">RFI Sections</h3><button class="btn btn-primary btn-sm" data-action="rfiAddSection">Add Section</button></div><div id="rfi-sections-list">${rows || '<div class="text-base-content/70 text-sm text-center py-3">No sections yet</div>'}</div></div></div>
  <script>
  ...
  window.rfiPersistOrder=function(list){var order=[].slice.call(list.children).map(function(el,i){return{id:el.dataset.id,sort_order:i}});fetch('/api/rfi_section/reorder',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({engagement_id:rfiEngId,order:order})}).then(function(){showToast('Reordered','success')}).catch(function(){showToast('Reorder failed','error')})};
  window.rfiDrop=function(e){e.preventDefault();var tgt=e.target.closest('.rfi-section-row');if(!tgt||!dragSrc||tgt===dragSrc)return;var list=tgt.parentNode;var all=[].slice.call(list.children);var srcIdx=all.indexOf(dragSrc);var tgtIdx=all.indexOf(tgt);if(srcIdx<tgtIdx)list.insertBefore(dragSrc,tgt.nextSibling);else list.insertBefore(dragSrc,tgt);window.rfiPersistOrder(list)};
  window.rfiSectionKey=function(e){var row=e.target.closest('.rfi-section-row');if(!row)return;if((e.ctrlKey||e.metaKey)&&(e.key==='ArrowUp'||e.key==='ArrowDown')){e.preventDefault();var list=row.parentNode;if(e.key==='ArrowUp'&&row.previousElementSibling){list.insertBefore(row,row.previousElementSibling)}else if(e.key==='ArrowDown'&&row.nextElementSibling){list.insertBefore(row.nextElementSibling,row)}else{return}row.focus();window.rfiPersistOrder(list)}};
  window.rfiAddSection=async function(){var name=await window.gmPrompt({title:'Add Section',label:'Section name',confirmLabel:'Add'});if(!name)return;fetch('/api/rfi_section',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,engagement_id:rfiEngId,sort_order:99})}).then(function(r){if(r.ok){showToast('Section added','success');location.reload()}else showToast('Failed','error')}).catch(function(){showToast('Error','error')})};
  window.rfiEditSection=async function(id,name){var n=await window.gmPrompt({title:'Rename Section',label:'Section name',value:name,confirmLabel:'Save'});if(!n||n===name)return;fetch('/api/rfi_section/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:n})}).then(function(r){if(r.ok){showToast('Updated','success');location.reload()}else showToast('Failed','error')}).catch(function(){showToast('Error','error')})};
  window.rfiDeleteSection=async function(id){if(!(await window.gmConfirm({title:'Delete Section',message:'Delete this section? This cannot be undone.',confirmLabel:'Delete',danger:true})))return;fetch('/api/rfi_section/'+id,{method:'DELETE'}).then(function(r){if(r.ok){showToast('Deleted','success');location.reload()}else showToast('Failed','error')}).catch(function(){showToast('Error','error')})};
  </script>`;
}
export function rfiQuestionEditDialog(questionId) {
  return `<div id="rfi-q-edit" class="dialog-overlay" style="display:none" data-dialog-close="rfi-q-edit" role="dialog" aria-modal="true" aria-labelledby="rfi-q-edit-title" aria-hidden="true">
    <div class="dialog-panel" style="max-width:640px">
      <div class="dialog-header"><span class="dialog-title" id="rfi-q-edit-title">Edit Question</span><button class="dialog-close" data-dialog-close="rfi-q-edit" aria-label="Close dialog">&times;</button></div>
      ...
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-q-edit">Cancel</button><button class="btn btn-primary btn-sm" data-action="rqeSave">Save</button></div>
    </div></div>
  <script>
  var rqeId=null;
...
export function rfiClientUserAssignment(rfiId) {
  return `<div id="rfi-assign-dialog" class="dialog-overlay" style="display:none" data-dialog-close="rfi-assign-dialog" role="dialog" aria-modal="true" aria-labelledby="rfi-assign-dialog-title" aria-hidden="true">
    <div class="dialog-panel"><div class="dialog-header"><span class="dialog-title" id="rfi-assign-dialog-title">Assign Client User</span><button class="dialog-close" data-dialog-close="rfi-assign-dialog" aria-label="Close dialog">&times;</button></div>
      ...
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-assign-dialog">Cancel</button><button class="btn btn-primary btn-sm" data-action="rcaAssign">Assign</button></div>
    </div></div>
  <script>
...
export function rfiBulkAssignDialog(rfiId) {
  return `<div id="rfi-bulk-assign" class="dialog-overlay" style="display:none" data-dialog-close="rfi-bulk-assign" role="dialog" aria-modal="true" aria-labelledby="rfi-bulk-assign-title" aria-hidden="true">
    <div class="dialog-panel"><div class="dialog-header"><span class="dialog-title" id="rfi-bulk-assign-title">Bulk Assign Client Users</span><button class="dialog-close" data-dialog-close="rfi-bulk-assign" aria-label="Close dialog">&times;</button></div>
      <div class="dialog-body"><div class="modal-form-group"><label for="rba-user">Client User</label><select id="rba-user" class="select select-bordered w-full"></select></div><div class="modal-form-group"><label class="flex items-center gap-2"><input type="checkbox" id="rba-unassigned" class="checkbox" checked/><span class="text-sm">Unassigned questions only</span></label></div><div id="rba-preview" class="text-sm text-base-content/70 mt-2"></div></div>
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-bulk-assign">Cancel</button><button class="btn btn-primary btn-sm" data-action="rbaBulk">Assign All</button></div>
    </div></div>
  <script>
...
export function rfiBulkDeadlineDialog(rfiId) {
  return `<div id="rfi-bulk-deadline" class="dialog-overlay" style="display:none" data-dialog-close="rfi-bulk-deadline" role="dialog" aria-modal="true" aria-labelledby="rfi-bulk-deadline-title" aria-hidden="true">
    <div class="dialog-panel"><div class="dialog-header"><span class="dialog-title" id="rfi-bulk-deadline-title">Set Bulk Deadline</span><button class="dialog-close" data-dialog-close="rfi-bulk-deadline" aria-label="Close dialog">&times;</button></div>
      ...
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-bulk-deadline">Cancel</button><button class="btn btn-primary btn-sm" data-action="rbdSet">Set Deadline</button></div>
    </div></div>
  <script>
...
export function rfiAttachmentPreview() {
  return `<div id="rfi-attach-preview" class="dialog-overlay" style="display:none" data-dialog-close="rfi-attach-preview" role="dialog" aria-modal="true" aria-labelledby="rfi-attach-preview-title" aria-hidden="true">
    <div class="dialog-panel" style="max-width:800px;max-height:90vh">
      <div class="dialog-header"><span class="dialog-title" id="rfi-attach-preview-title">Attachment</span><button class="dialog-close" data-dialog-close="rfi-attach-preview" aria-label="Close dialog">&times;</button></div>
      <div class="dialog-body" style="overflow:auto;max-height:70vh"><div id="rap-content" class="text-center"><div class="skeleton" style="width:100%;min-height:200px" role="status" aria-label="Loading attachment"></div></div></div>
      <div class="dialog-footer"><a id="rap-download" href="#" download class="btn btn-primary btn-sm">Download</a><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-attach-preview">Close</button></div>
    </div></div>
  <script>
  window.previewAttachment=function(url,name,type){document.getElementById('rfi-attach-preview').style.display='flex';document.getElementById('rfi-attach-preview-title').textContent=name||'Attachment';document.getElementById('rap-download').href=url;var c=document.getElementById('rap-content');var skel='<div class="skeleton" style="width:100%;min-height:200px" role="status" aria-label="Loading attachment"></div>';if(type&&type.startsWith('image/')){c.innerHTML=skel;var img=new Image();img.alt=name||'Attachment';img.style.maxWidth='100%';img.style.maxHeight='60vh';img.onload=function(){c.innerHTML='';c.appendChild(img)};img.onerror=function(){c.innerHTML='<div class="text-base-content/70 py-8">Could not load image preview.</div>'};img.src=url}else if(type==='application/pdf'){c.innerHTML=skel;var f=document.createElement('iframe');f.style.width='100%';f.style.height='60vh';f.style.border='none';f.onload=function(){c.innerHTML='';c.appendChild(f)};f.src=url}else{c.innerHTML='<div class="text-base-content/70 py-8"><div style="font-size:3rem" aria-hidden="true">&#128196;</div><div class="mt-2">'+name+'</div><div class="text-xs mt-1">Preview not available for this file type</div></div>'}};
...
export function rfiReportGeneration(rfiId, rfiName) {
  return `<div id="rfi-report-dialog" class="dialog-overlay" style="display:none" data-dialog-close="rfi-report-dialog" role="dialog" aria-modal="true" aria-labelledby="rfi-report-dialog-title" aria-hidden="true">
    <div class="dialog-panel"><div class="dialog-header"><span class="dialog-title" id="rfi-report-dialog-title">Generate RFI Report</span><button class="dialog-close" data-dialog-close="rfi-report-dialog" aria-label="Close dialog">&times;</button></div>
      <div class="dialog-body">
      ...
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-report-dialog">Cancel</button><button class="btn btn-primary btn-sm" data-action="rrgGenerate">Generate</button></div>
    </div></div>
  <script>
...
export function rfiEmailRemindersDialog(rfiId) {
  return `<div id="rfi-reminders-dialog" class="dialog-overlay" style="display:none" data-dialog-close="rfi-reminders-dialog" role="dialog" aria-modal="true" aria-labelledby="rfi-reminders-dialog-title" aria-hidden="true">
    <div class="dialog-panel"><div class="dialog-header"><span class="dialog-title" id="rfi-reminders-dialog-title">Email Reminders</span><button class="dialog-close" data-dialog-close="rfi-reminders-dialog" aria-label="Close dialog">&times;</button></div>
      <div class="dialog-body">
        ...
        <button class="btn btn-outline btn-sm mt-2" data-action="rreSendNow">Send Reminder Now</button>
      </div>
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-reminders-dialog">Cancel</button><button class="btn btn-primary btn-sm" data-action="rreSave">Save Settings</button></div>
    </div></div>
  <script>
...
export function rfiDataGridGrouping(questions = [], groupBy = 'section') {
  ...
  const groupRows = Object.entries(groups).map(([name, items]) => {
    const qRows = items.map(q => {
      const statusCls = q.status === 'answered' ? 'text-green-600' : q.status === 'overdue' ? 'text-red-600' : 'text-base-content/70';
      const qIdArg = esc(JSON.stringify([String(q.id)]));
      return `<tr class="hover cursor-pointer focus-visible:ring-2 focus-visible:ring-offset-2" tabindex="0" role="button" data-action="openQuestionEdit" data-args='${qIdArg}'><td class="text-sm">${esc(q.question_text || q.question || '-')}</td><td><span class="${statusCls}">${statusLabel(q.status || 'pending')}</span></td><td class="text-xs text-base-content/70">${esc(q.assigned_to_name || q.assigned_to || '-')}</td><td class="text-xs">${q.deadline ? new Date(q.deadline * 1000).toLocaleDateString('en-ZA',{day:'2-digit',month:'short',year:'numeric'}) : '-'}</td></tr>`;
    }).join('');
    return `<tr class="dg-group-header focus-visible:ring-2 focus-visible:ring-offset-2" tabindex="0" data-action="toggleNextSibling" data-self><td colspan="4"><button class="dg-expand-btn" aria-label="Toggle group">[+]</button><strong>${esc(name)}</strong><span class="badge-status ml-2">${items.length}</span></td></tr><tbody>${qRows}</tbody>`;
  }).join('');
...
export function rfiItemDialog() {
  return `<div id="rfi-item-dialog" class="dialog-overlay" style="display:none" data-dialog-close="rfi-item-dialog" role="dialog" aria-modal="true" aria-labelledby="rfi-item-dialog-title" aria-hidden="true">
    <div class="dialog-panel" style="max-width:640px"><div class="dialog-header"><span class="dialog-title" id="rfi-item-dialog-title">RFI Item</span><button class="dialog-close" data-dialog-close="rfi-item-dialog" aria-label="Close dialog">&times;</button></div>
      <div class="dialog-body">
      ...
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-item-dialog">Cancel</button><button class="btn btn-primary btn-sm" data-action="ridSave">Save</button></div>
    </div></div>
  <script>
...
export function rfiAttachDialog(rfiId) {
  return `<div id="rfi-attach-dialog" class="dialog-overlay" style="display:none" data-dialog-close="rfi-attach-dialog" role="dialog" aria-modal="true" aria-labelledby="rfi-attach-dialog-title" aria-hidden="true">
    <div class="dialog-panel"><div class="dialog-header"><span class="dialog-title" id="rfi-attach-dialog-title">Attach File to RFI</span><button class="dialog-close" data-dialog-close="rfi-attach-dialog" aria-label="Close dialog">&times;</button></div>
      <div class="dialog-body">
      ...
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-attach-dialog">Cancel</button><button class="btn btn-primary btn-sm" data-action="radUpload">Upload</button></div>
    </div></div>
  <script>
...
export function rfiAttachQuestionDialog(questionId) {
  return `<div id="rfi-aq-dialog" class="dialog-overlay" style="display:none" data-dialog-close="rfi-aq-dialog" role="dialog" aria-modal="true" aria-labelledby="rfi-aq-dialog-title" aria-hidden="true">
    <div class="dialog-panel"><div class="dialog-header"><span class="dialog-title" id="rfi-aq-dialog-title">Attach File to Question</span><button class="dialog-close" data-dialog-close="rfi-aq-dialog" aria-label="Close dialog">&times;</button></div>
      ...
      <div class="dialog-footer"><button class="btn btn-ghost btn-sm" data-dialog-close="rfi-aq-dialog">Cancel</button><button class="btn btn-primary btn-sm" data-action="raqUpload">Upload</button></div>
    </div></div>
  <script>
  var raqQid=null;
