...
export function engagementFileSearchDialog(engagementId) {
  ...
  <script>
  var efsTimer=null;
  window.efsSearch=function(){clearTimeout(efsTimer);efsTimer=setTimeout(function(){var q=document.getElementById('efs-query').value.trim();var el=document.getElementById('efs-results');if(!q){el.textContent='';return}fetch('/api/file?engagement_id=${engagementId}&search='+encodeURIComponent(q)).then(function(r){return r.json()}).then(function(d){var files=d.data||d||[];el.textContent='';if(!files.length){var none=document.createElement('div');none.className='text-gray-500 text-sm text-center py-4';none.textContent='No files found';el.appendChild(none);return}files.forEach(function(f){var row=document.createElement('div');row.className='flex items-center justify-between p-2 hover:bg-gray-50 rounded cursor-pointer';row.setAttribute('data-action','previewAttachment');row.setAttribute('data-args',JSON.stringify(['/api/file/'+f.id+'/download',f.name||'',f.mime_type||'']));var left=document.createElement('div');var nm=document.createElement('div');nm.className='text-sm font-medium';nm.textContent=f.name||'Unknown';var sz=document.createElement('div');sz.className='text-xs text-gray-500';sz.textContent=f.size?Math.round(f.size/1024)+'KB':'';left.appendChild(nm);left.appendChild(sz);var badge=document.createElement('span');badge.className='badge-status text-xs';badge.textContent=f.type||'file';row.appendChild(left);row.appendChild(badge);el.appendChild(row)})}).catch(function(){el.textContent='';var err=document.createElement('div');err.className='text-red-500 text-sm';err.textContent='Search error';el.appendChild(err)})},300)};
  </script>`;
}
...
export function postRfiFileUpload(engagementId) {
  ...
  <script>
  window.prfiUpload=async function(){var files=document.getElementById('prfi-files').files;if(!files.length){showToast('Select files','error');return}var prog=document.getElementById('prfi-progress');prog.textContent='Uploading '+files.length+' file(s)...';var ok=0;for(var i=0;i<files.length;i++){var fd=new FormData();fd.append('file',files[i]);fd.append('engagement_id','${engagementId}');fd.append('type','post_rfi');try{var r=await fetch('/api/file/upload',{method:'POST',body:fd});if(r.ok)ok++;prog.textContent='Uploaded '+(i+1)+' of '+files.length}catch(e){}}prog.textContent=ok+' of '+files.length+' uploaded successfully';if(ok>0)showToast(ok+' files uploaded','success')};
  </script>`;
}
