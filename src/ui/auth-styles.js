export const AUTH_CSS = `<style>
  ...
  .auth-logo-mark{display:inline-flex;align-items:center;justify-content:center;width:52px;height:52px;background:var(--color-primary,#04141f);border-radius:14px;margin-bottom:14px;box-shadow:0 4px 12px rgba(4,20,31,0.25)}
  ...
  .auth-submit{width:100%;padding:12px;background:var(--color-primary,#04141f);color:var(--panel-accent-fg,#fff);border:none;border-radius:8px;font-size:0.9375rem;font-weight:600;cursor:pointer;transition:all 0.15s;min-height:46px}
  .auth-submit:hover:not(:disabled){background:var(--color-primary-hover,#0a2535);box-shadow:0 4px 12px rgba(4,20,31,0.2)}
...
// No @media(prefers-color-scheme:dark) block here: every color above already
// reads var(--color-*), which the styles2.css bridge repoints to the real
// 247420 --bg/--fg/--panel-* tokens driven by the <html data-theme> attribute
// (layout.js's inline bootstrap script sets it from localStorage, falling
// back to the OS preference only on first visit). A parallel prefers-color-
// scheme block here fought that: it fired on OS dark preference regardless
// of the resolved data-theme, forcing this card dark even under
// data-theme="paper" (witnessed: near-black card + near-invisible labels on
// the light theme, screenshotted on /login with OS dark mode on).
